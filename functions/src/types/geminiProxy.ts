// Request and response types for the geminiProxy callable Cloud Function.
//
// Defined here (under functions/) so the next ticket can copy or re-export
// these into src/ for the client-side rewrite of geminiService.ts. Both
// sides of the wire stay in lockstep that way.
//
// Discriminated unions: switch on `queryType` for requests and `kind` for
// responses. New query types (image-based queries for MC-012, etc.) extend
// the request union and add a new switch case in geminiProxy.ts — no new
// callable.
